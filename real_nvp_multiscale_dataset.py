    ...
    with tf.variable_scope(name) as scope:
        ...
        if use_batch_norm:
            res = batch_norm(
                ...
                train=train, epsilon=1e-4, axes=[0, 1, 2])
            res *= 2.
        res = tf_concat_v2([res, -res], 3)
        res = tf_concat_v2([res, mask], 3)
        dim_in = 2. * channels + 1
        res = tf.nn.relu(res)
    ...
    with tf.variable_scope(name) as scope:
        ...
        if reverse:
            ...
            if use_batch_norm:
                ...
                log_diff -= .5 * log_var
        if change_bottom:
            res = tf_concat_v2([input_, res], 3)
            log_diff = tf_concat_v2([tf.zeros_like(log_diff), log_diff], 3)
        else:
            res = tf_concat_v2([res, input_], 3)
            log_diff = tf_concat_v2([log_diff, tf.zeros_like(log_diff)], 3)
    ...
    if scale_idx < (n_scale - 1):
        ...
        if FLAGS.recursion_type > 1:
            ...
            res_1, inc_log_diff = rec_masked_conv_coupling(
                ...
                train=train)
            res = tf_concat_v2([res_1, res_2], 3)
            log_diff_1 += inc_log_diff
            log_diff = tf_concat_v2([log_diff_1, log_diff_2], 3)
    ...
    def __init__(self, hps, sampling=False):
        ...
        if FLAGS.dataset == "imnet":
            ...
            with tf.device(
                ...
                image = tf.reshape(image, tf_concat_v2([height, width, depth], 0))
        ...
        metrics = dt.run_all_metrics(logit_x_in, z_out_as_mat, log_diff,
                                     gen_obs, gen_latent,
                                     sigma_list_latent, sigma_list_latent)
        ...
        if not sampling:
            ...
            for metric_name, val in metrics.iteritems():
                tf.summary.scalar(metric_name, tf.reduce_mean(val))
        ...
        if sampling:
            ...
            z_compressed = tf_concat_v2(z_compressed_list, 0)
            z_noisy = tf_concat_v2(z_noisy_list, 0)
